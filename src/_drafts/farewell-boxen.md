...
This sorcery is caused by GitHub&rsquo;s [custom build of NGINX][boxen nginx]: it clobbers `localhost:80` [by default][boxen nginx port 80], and all `*.dev` requests get resolved to it via [dnsmasq][boxen dnsmasq]. Basically&mdash;should you not be employed by GitHub&mdash;expect to learn ([and undo][remove nginx dnsmasq]!) some of Boxen&rsquo;s magic.
